...
import { AgentOptions } from "@/utility/SelectOptions";

const AgentLedgerPage = () => {
  const [dateFrom, setDateFrom] = useState(null);
  const [dateTo, setDateTo] = useState(null);
  const [agentLedgers, setAgentLedgers] = useState([]);
  ...
  const handleAgentChange = async (selectedOption) => {
    ...
    try {
      ...
      const res = await axios.get(`/agent-ledgers/${id}/agent-ledgers?${queryData}`);
      console.log(res);
      setAgentLedgers(res?.data);
                ...
                <table className="table container min-w-full">
                  ...
                  <tbody>
                    <tr className="border-b hover:bg-gray-100">
                      <td className="py-4">{agentLedgers[0]?.agent?.id}</td>
                      <td>{agentLedgers[0]?.agent?.nameEn}</td>{" "}
                      <td>{agentLedgers[0]?.agent?.total_balance}</td>
                      <td>{agentLedgers[0]?.agent?.commissionPercentage}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
